...
interface NetService {
    ...
    fun getAndroid(@NotNull @Path("type") type: String,
                   @Path("page") page: Int?,
                   @Path("pagesize") pagesize: Int?): Observable<JsonResult<MutableList<AndMol>>>

    /**
     * 视频列表
     */
    @GET("v2/categories?udid=26868b32e808498db32fd51fb422d00175e179df&vc=83")
    fun getVideoList(): Observable<MutableList<VideoMol>>
    ...
    fun getVideoMoreList(@Query("categoryName") categoryName: String, @Query("strategy") strategy: String,
                         @Query("udid") udid: String, @Query("vc") vc: Int): Observable<VideoMoreMol>
    ...
    fun getVideoMoreList1(@Query("start") start :Int,@Query("num") num :Int,
                          @Query("categoryName") categoryName :String,@Query("strategy") strategy :String): Observable<VideoMoreMol>
}